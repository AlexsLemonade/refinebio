...
def download_array_express(job_id: int) -> None:
    ...
    for extracted_file in extracted_files:
        ...
        if sample_objects.count() > 1:
            logger.warn(
                "Found an Array Express OriginalFile with more than one sample",
        ...
        if extracted_file["filename"].upper()[-4:] == ".CEL" and sample_object.has_raw:
            ...
            if platform_accession_code == "UNSUPPORTED":
                logger.error(
                    "Found a raw .CEL file with an unsupported platform!",
    ...
    if success:
        logger.debug("File downloaded and extracted successfully.", url=url, downloader_job=job_id)
