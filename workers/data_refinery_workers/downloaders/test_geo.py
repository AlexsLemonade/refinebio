...
class DownloadGeoTestCase(TestCase):
    ...
    def test_download_and_extract_file(self):
        ...
        self.assertTrue(
            os.path.isfile("/home/user/data_store/GSE10241/raw/GSE10241_family.xml.tar")
        )
    ...
    def test_download_aspera_and_ftp(self):
        ...
        dl_file_path = (
            ...
            + "/"
            + original_file.source_url.split("/")[-1]
        )

        # Aspera
        result = geo._download_file(
            original_file.source_url, file_path=dl_file_path, job=dlj, force_ftp=False
        )
        ...
        # FTP
        result = geo._download_file(
            original_file.source_url, file_path=dl_file_path, job=dlj, force_ftp=True
        )
        self.assertTrue(result)
        self.assertTrue(os.path.exists(dl_file_path))
        os.remove(dl_file_path)
        ...
        # Aspera, fail
        result = geo._download_file_aspera(
            "https://rich.zone/cool_horse.jpg",
