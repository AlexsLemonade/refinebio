...
def _prepare_files(job_context: Dict) -> Dict:
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "Expected text file, was this in fact a binary file?",
        ...
        if "ID_REF" not in joined and not job_context["is_illumina"]:
            try:
                ...
                logger.exception(
                    "Unable to read input file or header row.",
            ...
            if job_context["is_illumina"]:
                try:
                    ...
                    if len(row) > 3:
                        ...
                        logger.error(
                            e_msg, job_id=job_context["job"].pk, file=job_context["input_file_path"]
                        )
                        job_context["success"] = False
        ...
        job_context["internal_accession"] = get_internal_microarray_accession(
...
def _convert_affy_genes(job_context: Dict) -> Dict:
    ...
    if not os.path.exists(gene_index_path):
        ...
        job_context["job"].failure_reason = (
            "Missing gene index for " + job_context["internal_accession"]
        )
        job_context["job"].no_retry = True
        job_context["success"] = False
        return job_context
    ...
    try:
        ...
        error_template = (
            "Encountered error in R code while running {0}"
            " pipeline during processing of {1}: {2}"
        )
        error_message = error_template.format(
            job_context["script_name"], job_context["input_file_path"], str(e)
        )
        logger.error(error_message, job_context=job_context)
        job_context["job"].failure_reason = error_message
        job_context["success"] = False
