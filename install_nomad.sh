...
curl -0s https://releases.hashicorp.com/nomad/0.8.3/nomad_0.8.3_linux_amd64.zip > nomad_0.8.3_linux_amd64.zip
curl -0s https://releases.hashicorp.com/nomad/0.8.3/nomad_0.8.3_SHA256SUMS > nomad_0.8.3_SHA256SUMS
curl -0s https://releases.hashicorp.com/nomad/0.8.3/nomad_0.8.3_SHA256SUMS.sig > nomad_0.8.3_SHA256SUMS.sig

# Verify the signature file is untampered.
gpg_ok=$(gpg --verify nomad_0.8.3_SHA256SUMS.sig nomad_0.8.3_SHA256SUMS |& grep Good)
if [[ "$gpg_ok" = "" ]]; then
...
shasum_ok=$(sha256sum -c nomad_0.8.3_SHA256SUMS |& grep OK)
if [[ "$shasum_ok" = "" ]]; then
...
rm nomad_0.8.3_SHA256SUMS
rm nomad_0.8.3_SHA256SUMS.sig
