...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        samples_without_downloader = (
            ...
            .filter(
                is_processed=False, original_files_count__gt=0, downloader_job_count=0
            )
        )
        ...
        samples_without_downloader = samples_without_downloader.prefetch_related(
            "original_files"
        )
        ...
        while True:
            for sample in page.object_list:
                logger.debug(
                    "Creating downloader job for a sample.",
                    sample=sample.accession_code,
                )
                create_downloader_job(sample.original_files.all())

            logger.info(
                "Created %d new downloader jobs because their samples didn't have any.",
                PAGE_SIZE,
            )
